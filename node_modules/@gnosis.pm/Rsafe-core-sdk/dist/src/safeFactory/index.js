...
class SafeFactory {
    ...
    async init({ ethAdapter, safeVersion, isL1SafeMasterCopy, contractNetworks }) {
        ...
        __classPrivateFieldSet(this, _SafeFactory_safeProxyFactoryContract, await this.getProxyFactoryContract({
            ethAdapter,
            safeVersion,
            chainId,
            customContracts
        }), "f");
        __classPrivateFieldSet(this, _SafeFactory_gnosisSafeContract, await this.getSafeContract({
            ethAdapter,
            safeVersion,
            chainId,
            isL1SafeMasterCopy,
    ...
    async deploySafe({ safeAccountConfig, safeDeploymentConfig, options }) {
        var _a;
    ...
    async getSafeContract({ ethAdapter, safeVersion, chainId, isL1SafeMasterCopy, customContracts }) {
        const safeSingletonDeployment = (0, safeDeploymentContracts_1.getSafeContractDeployment)(config_1.SAFE_LAST_VERSION, chainId, isL1SafeMasterCopy);
        const gnosisSafeContract = ethAdapter.getSafeContract({
            safeVersion: safeVersion,
            chainId,
            singletonDeployment: safeSingletonDeployment,
            customContractAddress: customContracts === null || customContracts === void 0 ? void 0 : customContracts.safeMasterCopyAddress,
            customContractAbi: customContracts === null || customContracts === void 0 ? void 0 : customContracts.safeMasterCopyAbi
        });
        const isContractDeployed = await __classPrivateFieldGet(this, _SafeFactory_ethAdapter, "f").isContractDeployed(gnosisSafeContract.getAddress());
        if (!isContractDeployed) {
            throw new Error('Safe Proxy contract is not deployed in the current network');
        }
        return gnosisSafeContract;
    }
    async getProxyFactoryContract({ ethAdapter, safeVersion, chainId, customContracts }) {
        ...
        const isContractDeployed = await __classPrivateFieldGet(this, _SafeFactory_ethAdapter, "f").isContractDeployed(safeProxyFactoryContract.getAddress());
        if (!isContractDeployed) {
